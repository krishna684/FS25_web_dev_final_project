...
const Navbar = () => {
  ...
  return (
    <nav
      className="navbar bg-[var(--navbar-bg)] border-b-4 border-[var(--primary)] sticky top-0 z-50 shadow-md"
      style={{
        transition:
    ...
    >
      <div className="container max-w-[1250px] mx-auto flex justify-between items-center px-3 py-3">
        ...
        <div
          ref={navLeftRef}
          className={`nav-left flex items-center flex-1 ${
        ...
        {/* CENTER - Search Bar (spacing unchanged) */}
        <div
          ref={navCenterRef}
          className={`nav-center flex justify-center items-center mx-0 transition-all gap-3 ${
            mobileSearchOpen ? "flex-[1] w-full" : "flex-[8] hidden md:flex"
          }`}
        >
          ...
          <button
            ...
            className={`icon-btn ${
              mobileSearchOpen ? "inline-flex" : "hidden"
            }`}
            title="Close search"
          >
            <ArrowLeft size={18} />
          ...
          {/* Search Input */}
          <div className="relative flex items-center w-full max-w-[600px] gap-3">
            <input
              type="text"
              value={query}
              ...
              className="flex-1 bg-[var(--bg-surface)] border border-[var(--primary)] rounded-lg pl-3 pr-3 py-1.5 outline-none text-[var(--text-main)] text-base placeholder-[var(--text-tertiary)]"
              style={{
                color: "var(--text-main)",
        ...
        <div
          ref={navRightRef}
          className={`nav-right flex justify-end items-center gap-2 flex-[3] ${
            mobileSearchOpen ? "hidden" : "flex"
          }`}
        >
          {/* Mobile Search Icon */}
