...
const TaskDetailModal = ({ task, isOpen, onClose, onUpdate, onDelete }) => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                const res = await commentApi.getForTask(task._id);
    ...
    const handleAddComment = async ({ text }) => {
        ...
        setCommentError('');
        try {
            await commentApi.addToTask(task._id, text);
            const res = await commentApi.getForTask(task._id);
            setComments(res.data || []);
        } catch (err) {
                ...
                >
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="border-t border-[var(--border)] pt-6">
                            ...
                            {commentError && (
                                <p className="text-sm text-red-600 mb-3">{commentError}</p>
                            )}

                            {commentsLoading ? (
                                <p className="text-sm text-[var(--text-tertiary)] italic">Loading comments…</p>
                            ) : (
